...
f = xP3(5); %nA-1
p_d = xP3(6); %ms-1
p_p = rate_p*xP3(7); %ms-1
M_p = rate_p*xP3(8); %nA/msec
M_d = xP3(9); %nA/ms
A_p = rate_p*xP3(10); %nA2
A_d = xP3(11); %nA2
C_m_p = xP3(12); % pF
n_syn = xP3(13);
...
I_caNMDA = 0.1*alpha*g_NMDA*(V-20);% NOTE V-Erev cancelled

dCdt =  (-gamma*stim_ON*I_caNMDA - eta*C); 
dNpdt = (nu*C -  (p_p - I_caNMDA*stim_ON*g)*Np +  (M_p*Np^2)/(A_p+Np^2));
dNddt = (nu*C - (p_d - I_caNMDA*stim_ON*g)*Nd + (M_d*Nd^2)/(A_d+Nd^2)); 
...
dxdt = [dVdt,dhNaT,dnKDR,dbKA,duKM,drCa,dCaidt,dcCa,dqCa,dh_AMPAdt,dU_AMPAdt,...
    dh_NMDAdt,dU_NMDAdt,dCdt,dNpdt,dNddt,dTPKA_ONdt,dTPKA_OFFdt,dTPLC_ONdt,...
    dTPLC_OFFdt,dTLTP_ONdt,dTLTD_ONdt,dtag_stim_offdt,dtag_stimdt,dtag_DA_offdt...
